...
namespace WoWonder.SocketSystem
{
	public class AppNotificationsManager
	{
		...
		public void ShowUserNotification(Context mContext, string type, string conversationId, string username, string message, string id, string ChatId, string avatar, string color, int counterUnreadMessages = 1)
		{
			try
			{
				string channelId;
				...
				Intent messagingIntent = new Intent(Application.Context, typeof(TabbedMainActivity));
				...
				SetupBadger(mContext, counterUnreadMessages);
			}
		...
		NotificationCompat.Builder MNotifyBuilder;
		public void ShowUpDownNotification(Context mContext, string UserName, string messageId, string userId, string ChatId)
		{
			try
			{
				string channelId;
		...
		/// </summary>
		public void SetupBadger(Context mContext, int messageBadgeCounter = 0)
		{
