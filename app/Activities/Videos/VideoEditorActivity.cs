...
namespace WoWonder.Activities.Videos
{
    ...
    public class VideoEditorActivity : BaseActivity, MaterialDialog.IListCallback, VideoSeekBarView.ISeekBarDelegate, VideoTimelineView.IVideoTimelineViewListener, MediaPlayer.IOnPreparedListener, MediaPlayer.IOnCompletionListener
    {
        ...
        private void InitializerView()
        {
            try
            {
                ...
                FfMpeg.LoadBinary(new MyFFmpegLoadBinaryResponseHandler());
                 
                Runtime.GetRuntime()?.RunFinalization();
                Runtime.GetRuntime()?.Gc();
                ApiRequest.TrimCache(this); 
            }
            catch (Exception e)
            {
                Methods.DisplayReportResultTrack(e);
        ...
        private void FastMotionVideoCommand(long startMs, long endMs)
        {
            try
            {
                ...
                complexCommand.Add("mpeg4");

                //Set the video bitrate
                complexCommand.Add("-b:v");
                complexCommand.Add("2097k");
        ...
        private void ExecFFmpegBinary(string[] command)
        {
            try
            {
                FfMpeg.Execute(command, new MyExecuteBinaryResponseHandler(this));
            }
            catch (Exception e)
            {
                Methods.DisplayReportResultTrack(e);
        ...
        private class MyExecuteBinaryResponseHandler : ExecuteBinaryResponseHandler
        {
            private readonly VideoEditorActivity Activity;
            public MyExecuteBinaryResponseHandler(VideoEditorActivity activity)
            {
                Activity = activity;
            }
            ...
            public override void OnFailure(string p0)
            {
                ...
                {
                    base.OnFailure(p0);
                    Console.WriteLine("FAILED with output : " + p0);
                   
                    AndHUD.Shared.Dismiss(Activity);

                    ToastUtils.ShowToast(Activity, Activity.GetText(Resource.String.Lbl_ErrorCreatingVideo), ToastLength.Short);

                    Methods.DisplayReportResultTrack(new Exception(p0));
                }
                catch (Exception e)
                {
                    Methods.DisplayReportResultTrack(e);
            ...
            public override void OnSuccess(string p0)
            {
                try
                {
                    ...
                    var fileName = Activity.MTranscodeOutputFile.AbsolutePath.Split('/').Last();
                    ...
                    switch (videoPlaceHolderImage)
                    {
                        ...
                        {
                            var bitmapImage = Methods.MultiMedia.Retrieve_VideoFrame_AsBitmap(Activity, Activity.MTranscodeOutputFile.AbsolutePath);
                        ...
                        try
                        {
                            switch (Activity.Type)
                            {
                                case "Story":
                                {
                                    ...
                                    {
                                        StoryFileType = "video",
                                        StoryFilePath = Activity.MTranscodeOutputFile.AbsolutePath,
                                        StoryDescription = Activity.EmojisIconEditText.Text,
                                        StoryTitle = Activity.EmojisIconEditText.Text,
                                        StoryThumbnail = fullPathFile.Path,
                                    };

                                    Intent intent = new Intent(Activity, typeof(PostService));
                                ...
                                case "Post":
                                {
                                    ...
                                    resultIntent.PutExtra("VideoPath", Activity.MTranscodeOutputFile.AbsolutePath);
                                    resultIntent.PutExtra("VideoId", Activity.IdVideo);
            ...
            public override void OnProgress(string p0)
            {
                try
                {
                    base.OnProgress(p0);
        ...
        private class MyFFmpegLoadBinaryResponseHandler : Java.Lang.Object, IFFmpegLoadBinaryResponseHandler
        {
            public MyFFmpegLoadBinaryResponseHandler()
            {
