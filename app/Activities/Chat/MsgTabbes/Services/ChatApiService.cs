    ...
    public class PostUpdaterHelper : Java.Lang.Object, IRunnable
    {
        ...
        public static async Task LoadChatAsync()
        {
            try
            {
                ...
                else if (AppSettings.LastChatSystem == SystemGetLastChat.MultiTab && Methods.AppLifecycleObserver.AppState == "Foreground")
                {
                    var inst = MsgTabbedMainActivity.GetInstance();

                    if (AppSettings.EnableChatGroup)
                        inst?.LastGroupChatsTab?.StartApiService();

                    if (AppSettings.EnableChatPage)
                        inst?.LastPageChatsTab?.StartApiService();
                }
