        ...
        private void SuggestionAdapterOnItemClick(object sender, AdapterClickEvents e)
        {
            try
            {
                ...
                if (Methods.CheckConnectivity())
                {
                    ...
                    {
                        UserDetails.Socket?.EmitAsync_SendPageMessage(PageId, UserId, UserDetails.AccessToken, UserDetails.Username, SuggestionAdapter.GetItem(position).RealMessage, time2);
                    }
                    else
                    {
            ...
            try
            {
                ...
                else if (RecordButton?.Tag?.ToString() == "Text")
                {
                    ...
                    else
                    {
                        ...
                        if (Methods.CheckConnectivity())
                        {
                            if (AppSettings.ConnectionTypeChat == InitializeWoWonder.ConnectionType.Socket && string.IsNullOrEmpty(ReplyId))
                            {
                                UserDetails.Socket?.EmitAsync_SendPageMessage(PageId, UserId, UserDetails.AccessToken, UserDetails.Username, EmojIconEditTextView.Text, time2);
                            }
                            else
                            {
                                Task.Factory.StartNew(() =>
                            ...
                            case MessageModelType.RightSticker:
                                {
                                    ...
                                    SelectedItemPositions.MesData.Media = WoWonderTools.GetFile(PageId, Methods.Path.FolderDiskSticker, fileName, SelectedItemPositions.MesData.Media);
