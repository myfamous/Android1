...
{ 
    public class ViewReelsVideoFragment : AndroidX.Fragment.App.Fragment, IYouTubePlayerInitListener, IPlayerEventListener, StTools.IXAutoLinkOnClickListener
    {
        ...
        private LinearLayout UserLayout , GiftLayout, LikeLayout, CommentLayout, ShareLayout;
        public ImageView UserImageView , ImgSendGift, ImgLike, ImgComment, ImgShare;
        public TextView TxtUsername , TxtLikeCount, TxtCommentCount, TxtShareCount;
        MyReactButton LikeButton;
        ...
        private void InitComponent(View view)
        {
            try
            {
                ...
                LikeButton = MainView.FindViewById<MyReactButton>(Resource.Id.ReactButton);
                ...
                try {
                    DataVideos.Reaction ??= new WoWonderClient.Classes.Posts.Reaction(); } catch { }

                if (DataVideos.Reaction.IsReacted != null && DataVideos.Reaction.IsReacted.Value)
                {
                    switch (string.IsNullOrEmpty(DataVideos.Reaction.Type))
                    {
                        case false:
                            {
                                var react = ListUtils.SettingsSiteList?.PostReactionsTypes?.FirstOrDefault(a => a.Value?.Id == DataVideos.Reaction.Type).Value?.Id ?? "";
                                switch (react)
                                {
                                    case "1":
                ...
                LikeButton.Text = DataVideos.PostLikes + "";
        ...
        {
            LikeButton.ClickLikeAndDisLike(new GlobalClickEventArgs { NewsFeedClass = DataVideos, View = MainView }, null);
            changePostReactions();
        }
        void changePostReactions()
        {
            try
            {
                var item =TabbedMainActivity.GetInstance().NewsFeedTab.PostFeedAdapter.ListDiffer.Where(s => s.PostData?.Id == DataVideos.Id).FirstOrDefault();
                if (item != null)
                {
                    item.PostData.Reaction = DataVideos.Reaction;
                    item.PostData.PostLikes = DataVideos.PostLikes;
                    int indix = TabbedMainActivity.GetInstance().NewsFeedTab.PostFeedAdapter.ListDiffer.IndexOf(item);
                    //   context.NewsFeedTab.PostFeedAdapter.NotifyItemChanged(indix);
                    TabbedMainActivity.GetInstance().NewsFeedTab.PostFeedAdapter.NotifyDataSetChanged();
                }
            }
            catch { }
        ...
        private void LikeLayoutOnClick(object sender, EventArgs e)
        {
            try
            {
                new ReactionReelsVideo(Activity)?.ImgButtonOnClick(new ReactionsClickEventArgs { ImgButton = ImgLike, Position = 0, React = ReactConstants.Like },
                new GlobalClickEventArgs() { NewsFeedClass = DataVideos });
            }
            catch (Exception exception)
            {
                Methods.DisplayReportResultTrack(exception);
