...
namespace WoWonder.Helpers.Utils
{
    public static class WoWonderTools  
    {
        ...
        public static string IsJoinedGroup(GroupDataObject pageData)
        {
            try
            {
                //(0 => not joined , 1 =>joined , 2 =>  requested) 
                if (pageData.IsGroupJoined != null)
                {
                    return pageData.IsGroupJoined.Value.ToString();
                }
                 
                if (!string.IsNullOrEmpty(pageData?.IsJoined.String))
                {
                    if (pageData.IsJoined.String.ToLower() == "no")
                        return "0";
                    else if (pageData.IsJoined.String.ToLower() == "yes")
                        return "1";
                }

                if (pageData?.IsJoined.Bool != null && pageData.IsJoined.Bool.Value)
                {
                    return "1";
                }
                else
            ...
            try
            {
                ...
                switch (isFollowing)
                {
                    ...
                    case "0":
                        ...
                        btnAddUser.SetBackgroundResource(Resource.Drawable.round_button_normal2);
                        btnAddUser.Tag = "Add";
                        break;
                    //>> Friend
                    case "1":
                        ...
                        btnAddUser.SetBackgroundResource(Resource.Drawable.round_button_pressed2);
                        btnAddUser.Tag = "friends";
                        break;
                    //>> Request
                    case "2":
                    ...
                    case 200:
                        {
                            switch (respond)
                            {
                                case JoinGroupObject {JoinStatus: "requested"} result:
                                    button.SetTextColor(Color.ParseColor("#444444"));
                                    button.Text = Application.Context.GetText(Resource.String.Lbl_Request);
                                    button.SetBackgroundResource(Resource.Drawable.round_button_normal2);
                                    break;
                                case JoinGroupObject result:
                                    {
                                        var isJoined = result.JoinStatus == "left" ? "false" : "true";
